...
import { getTrips, getAllCatches, getAllConditions } from '../lib/storage';
...
export default function Analysis() {
  ...
  const [conditions, setConditions] = useState<TripConditions[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function load() {
      const [t, c, cond] = await Promise.all([getTrips(), getAllCatches(), getAllConditions()]);
      setTrips(t);
      setCatches(c);
      setConditions(cond);
      setLoading(false);
  ...
  const analysis = useMemo(() => {
    ...
    const tripStats = trips.map(t => {
      const tc = catches.filter(c => c.trip_id === t.id);
      const cond = conditions.find(c => c.trip_id === t.id);
      return {
        ...t,
        fishCount: tc.length,
        totalWeight: tc.reduce((s, c) => s + (c.weight_lbs || 0), 0),
        conditions: cond,
      };
    });
    ...
    return { lureEfficiency, monthlyStats, topPatterns };
  }, [trips, catches, conditions]);
    ...
    <div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-3">
        ...
        <div className="space-y-2">
          {analysis.lureEfficiency.slice(0, 8).map(l => (
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-3">
        ...
        <div className="space-y-1">
          {Object.entries(analysis.monthlyStats).sort((a, b) => b[0].localeCompare(a[0])).map(([month, stats]) => (
